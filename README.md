...
### 目前最新测试，wsl.conf都不需要再配置了。输入法也只需要装fcitx及fcitx-googlepinyin，配置切换键避免和Windows自带输入法冲突即可。
...
3. 任务栏搜索wt,进入后，输入wsl --update 对wsl进行升级以便支持Linux GUI功能（重要）。
...
某种系统级命令，可对服务开启重启停止等等，只需新建/etc/wsl.conf文件并写入以下内容保存重启wsl即可。
